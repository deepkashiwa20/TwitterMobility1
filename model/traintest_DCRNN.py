...
def getModel():
    ...
    # adj_mx = [sym_adj(A)] 
    adj_mx = [asym_adj(A), asym_adj(np.transpose(A))] # "double_transition"
    model = DCRNN(device, num_nodes=num_variable, input_dim=channel, out_horizon=opt.seq_len, P=adj_mx).to(device)
