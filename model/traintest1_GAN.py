...
import Metrics
from Utils import get_pref_id, get_flow, get_adj, get_twitter, get_onehottime, get_seq_data, sym_adj, getXSYS
...
def evaluate(D, G, x, y, adj, device):
    # validation and testing
    test_x, test_y, test_adj = torch.tensor(x).to(device), torch.tensor(y).to(device), torch.tensor(adj).to(device)
    ...
    YS, YS_pred = np.vstack(YS), np.vstack(YS_pred)
    ...
    return YS, YS_pred, D_losses_test, G_losses_test, truth, pred
...
def train(D, G, x, y, adj, device):
    num_train_sample = int(x.shape[0] * (1 - opt.val_ratio))
    train_x, train_y, train_adj = x[:num_train_sample, ...], y[:num_train_sample, ...], adj[:num_train_sample, ...]
    val_x, val_y, val_adj = x[num_train_sample:, ...], y[num_train_sample:, ...], adj[num_train_sample:, ...]
    logger.info('train_x.shape, train_y.shape, train_adj.shape', train_x.shape, train_y.shape, train_adj.shape)
    logger.info('val_x.shape, val_y.shape, val_adj.shape', val_x.shape, val_y.shape, val_adj.shape)
    ...
    for epoch in range(opt.epoch):
        ...
        # use validation data
        YS, YS_pred, D_losses_test, G_losses_test, truth, pred = evaluate(D, G, val_x, val_y, val_adj, device)
        rmse_test = np.sqrt(np.mean(np.square(YS - YS_pred)))
        YS, YS_pred = np.squeeze(YS), np.squeeze(YS_pred)
        for i in range(YS.shape[1]):
            YS[:, i, :], YS_pred[:, i, :] = scaler.inverse_transform(YS[:, i, :]), scaler.inverse_transform(YS_pred[:, i, :])
        ...
        logger.info('Epoch', epoch, time.ctime(), 'D_loss_train, G_loss_train, D_loss_test, G_loss_test, RMSE_test, RMSE, MAE, MAPE', \
                    D_losses_train, G_losses_train, D_losses_test, G_losses_test, rmse_test, RMSE, MAE, MAPE)
        
        if rmse_test < min_val_loss:
            min_val_loss = rmse_test
            torch.save(D.state_dict(), f'{path}/D_params.pt')
            torch.save(G.state_dict(), f'{path}/G_params.pt')  # save parameters
        else:
            pass
...
def test(D, G, x, y, adj, device):
    D.load_state_dict(torch.load(f'{path}/D_params.pt'))
    G.load_state_dict(torch.load(f'{path}/G_params.pt'))
    
    logger.info('test_x.shape, test_y.shape, test_adj.shape', x.shape, y.shape, adj.shape)
    YS, YS_pred, D_losses_test, G_losses_test, truth, pred = evaluate(D, G, x, y, adj, device)
    rmse_test = np.sqrt(np.mean(np.square(YS - YS_pred)))
    YS, YS_pred = np.squeeze(YS), np.squeeze(YS_pred)
    for i in range(YS.shape[1]):
        YS[:, i, :], YS_pred[:, i, :] = scaler.inverse_transform(YS[:, i, :]), scaler.inverse_transform(YS_pred[:, i, :])
    MSE, RMSE, MAE, MAPE = Metrics.evaluate(YS, YS_pred)
    with open(path + f'/{model_name}_prediction_scores.txt', 'a') as f:
        f.write("%s, %s, Original RMSE, %.10e\n" % (model_name, 'test', rmse_test))
        f.write("%s, %s, MSE, RMSE, MAE, MAPE, %.10f, %.10f, %.10f, %.10f\n" % (model_name, 'test', MSE, RMSE, MAE, MAPE))
    logger.info("%s, %s, Original RMSE, %.10e" % (model_name, 'test', rmse_test))
    logger.info("%s, %s, MSE, RMSE, MAE, MAPE, %.10f, %.10f, %.10f, %.10f" % (model_name, 'test', MSE, RMSE, MAE, MAPE))
...
parser.add_argument('--num_head', type=int, default=2, help='number of heads in self-attention') # should be fully divided by num_variable
...
parser.add_argument('--ex', type=str, default='covid-inflow-kanto8', help='which experiment setting to run')
...
path = f'./{exp}_{model_name}_{condition_flag}_head{opt.num_head}_' + time.strftime('%Y%m%d%H%M', time.localtime())
...
def main():  
    ...
    x, c, adj = flow, onehottime, adj
    
    x = scaler.fit_transform(x)
    
    x = x[:, :, np.newaxis].repeat(channel, axis=2) # final_feat=channel=1
    c = c[:, np.newaxis].repeat(num_variable, axis=1) # this is condition/label
    adj = sym_adj(adj)
    adj = adj[np.newaxis, :, :].repeat(x.shape[0], axis=0)
    
    _, train_x = getXSYS(x, 'train', opt.his_len, opt.seq_len, opt.trainval_ratio)
    _, train_c = getXSYS(c, 'train', opt.his_len, opt.seq_len, opt.trainval_ratio)
    _, train_adj = getXSYS(adj, 'train', opt.his_len, opt.seq_len, opt.trainval_ratio)
    _, test_x = getXSYS(x, 'test', opt.his_len, opt.seq_len, opt.trainval_ratio)
    _, test_c = getXSYS(c, 'test', opt.his_len, opt.seq_len, opt.trainval_ratio)
    _, test_adj = getXSYS(adj, 'test', opt.his_len, opt.seq_len, opt.trainval_ratio)
    
    logger.info(train_x.shape, train_c.shape, train_adj.shape, train_x.min(), train_x.max(), train_c.min(), train_c.max(), train_adj.min(), train_adj.max())
    logger.info(test_x.shape, test_c.shape, test_adj.shape, test_x.min(), test_x.max(), test_c.min(), test_c.max(), test_adj.min(), test_adj.max())
    ...
    logger.info(opt.D_init_feat, opt.cond_feat, opt.D_hidden_feat, opt.D_final_feat)
    logger.info(opt.G_init_feat, opt.cond_feat, opt.G_hidden_feat, opt.G_final_feat)
    ...
    time1 = time.ctime()
    train(D, G, train_x, train_c, train_adj, device)
    time2 = time.ctime()
    test(D, G, test_x, test_c, test_adj, device)
    time3 = time.ctime()
    logger.info('start and end time for training process...', time1, time2)
    logger.info('start and end time for total training process...', time2, time3)
