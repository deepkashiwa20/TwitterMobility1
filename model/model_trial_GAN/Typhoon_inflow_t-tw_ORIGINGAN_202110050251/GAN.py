...
class Generator(nn.Module):
    ...
    def __init__(self, init_features, cond_features, cond_sources, hidden_features, final_features, num_heads, dropout, num_block, num_variable, seq_len):
        super(Generator, self).__init__()
        self.num_block = num_block
        self.seq_len = seq_len
        self.num_variable = num_variable 
        
        # new added for transforming the conditions.
        self.cond_features = cond_features
        self.cond_sources = cond_sources
        self.linear_c = get_clones(nn.Linear(1, cond_features), cond_sources)
        
        self.DyConv1 = DyConv(init_features + cond_features*cond_sources, hidden_features)
    ...
    def forward(self, x, adj, c):
        ...
        if self.cond_sources == 1:
            pass
        elif self.cond_sources == 2:
            c0 = c[:, :, :, :-1]
            c1 = torch.tanh(self.linear_c[0](c[:, :, :, -1:]))
            c = torch.cat((c0, c1), dim=3)
        elif self.cond_sources == 3:
            c0 = c[:, :, :, :-2]
            c1 = torch.tanh(self.linear_c[0](c[:, :, :, -2:-1]))
            c2 = torch.tanh(self.linear_c[1](c[:, :, :, -1:]))
            c = torch.cat((c0, c1, c2), dim=3)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, init_features, cond_features, cond_sources, hidden_features, final_features, num_heads, dropout, num_block, num_variable, seq_len):
        ...
        self.cond_features = cond_features
        self.cond_sources = cond_sources
        self.linear_c = get_clones(nn.Linear(1, cond_features), cond_sources-1)
...
def main():
    ...
    D = Discriminator(D_init_feat, cond_feat, cond_sources, D_hidden_feat, final_feat, num_head, dropout, num_block, num_variable, seq_len).to(device)
    G = Generator(G_init_feat, cond_feat, cond_sources, G_hidden_feat, final_feat, num_head, dropout, num_block, num_variable, seq_len).to(device)
    adj = (seq_len, num_variable, num_variable)
    c = (seq_len, num_variable, cond_feat+cond_sources-1)
