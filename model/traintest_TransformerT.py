...
def getModel():
    # Here is different channel=1
    # original
    model = Transformer(in_channels=channel, embed_size=64, pe_length=opt.his_len, num_layers=1, timestep_in=opt.his_len, 
                        timestep_out=opt.seq_len, heads=8, forward_expansion=32, gpu=device, dropout=0).to(device)
    # big
    # model = Transformer(in_channels=channel, embed_size=64, pe_length=opt.his_len, num_layers=2, T_dim=opt.his_len, 
    #                     output_T_dim=opt.seq_len, heads=4, forward_expansion=64, gpu=device, dropout=0).to(device)
    return model
