...
def visualize_tweet_count_effectiveness(args: SimpleNamespace) -> None:
    ...
    axes = fig.subplots(2, 2, sharex=True, gridspec_kw={'hspace': 0.05, 'wspace': 0.05})
    visualize_partial_distance_correlation(args, axes[0, 0], typhoon_inflow, typhoon_tweet, typhoon_event_dates_index, args.typhoon_zoom_range, True, False, [0.55, 0.855], [0, 0.61], [0.1, 0.3, 0.5], [0.6, 0.7, 0.8])
    visualize_partial_distance_correlation(args, axes[1, 0], covid_inflow, covid_tweet, covid_event_dates_index, args.covid_zoom_range, True, False, [0.54, 0.91], [-0.01, 0.36], [0.05, 0.15, 0.25], [0.6, 0.7, 0.8])
    visualize_partial_distance_correlation(args, axes[0, 1], typhoon_outflow, typhoon_tweet, typhoon_event_dates_index, args.typhoon_zoom_range, False, True, [0.55, 0.855], [0, 0.61], [0.1, 0.3, 0.5], [0.6, 0.7, 0.8])
    ax1, ax2 = visualize_partial_distance_correlation(args, axes[1, 1], covid_outflow, covid_tweet, covid_event_dates_index, args.covid_zoom_range, False, True, [0.54, 0.91], [-0.01, 0.36], [0.05, 0.15, 0.25], [0.6, 0.7, 0.8])
    axes[0, 0].set_title('Inflow', fontsize=12)
    axes[0, 1].set_title('Outflow', fontsize=12)
    axes[0, 0].set_ylabel('2019 Typhoon\nSeason', fontsize=12, labelpad=20)
    axes[1, 0].set_ylabel('COVID-19', fontsize=12, labelpad=26)
    axes[0, 0].tick_params(axis='x', length=0)
    axes[0, 1].tick_params(axis='x', length=0)
    for ax in [axes[0, 0], axes[0, 1], axes[1, 0], axes[1, 1]]:
        ax.tick_params(axis='both', labelsize=8)
    # axes[-1, -1].legend()
    fig.text(
        0.5, 0., 'Days Before Events',
    ...
    fig.legend(handles=lhandles, loc='lower center', ncol=2, bbox_to_anchor=(0.5, -0.1), prop={'size': 8})
...
if __name__ == '__main__':
    ...
    config.read('src/IEEE_IJCAI2022_data_visualization/figure2/tweet_count_effectiveness.conf')
