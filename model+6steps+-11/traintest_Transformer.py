...
def getModel():
    model = Transformer(num_nodes=num_variable, in_channels=opt.channelin, out_channels=opt.channelout, n_his=opt.his_len, n_pred=opt.seq_len, hidden_channels=64, n_layers=3, 
                        device=device, stnorm_bool=False, snorm_bool=False, tnorm_bool=False).to(device)
    return model
