...
from Utils import get_pref_id, get_flow, get_adj, get_seq_data, getXSYS_single, getXSYS, get_onehottime, get_floatdaytime

def refineXSYS(XS, YS, TE):
    XS, YS, TE = XS[:, :, :, np.newaxis], YS[:, :, :, np.newaxis], TE[:, :, :, np.newaxis]
    XS = np.concatenate([XS, TE], axis=-1)
    return XS, YS

def getModel():
    # Here is different channel=2
    model = Transformer(in_channels = channel+1, 
                        embed_size = 64, 
                        time_num = opt.his_len, 
                        num_layers = 1, 
                        T_dim = opt.his_len, 
                        output_T_dim = opt.seq_len, 
                        heads = 8, 
...
def main():
    ...
    daytime = get_floatdaytime(target_start_date, target_end_date, freq)
    daytime = np.tile(daytime[:, np.newaxis], [1, num_variable])
    
    logger.info('original flow, daytime', flow.shape, flow.min(), flow.max(), daytime.shape)
    ...
    trainXS_TE, _ = getXSYS(daytime, 'train', opt.his_len, opt.seq_len, opt.trainval_ratio)
    trainXS, trainYS = refineXSYS(trainXS, trainYS, trainXS_TE)
    ...
    testXS_TE, _ = getXSYS(daytime, 'test', opt.his_len, opt.seq_len, opt.trainval_ratio)
    testXS, testYS = refineXSYS(testXS, testYS, testXS_TE)
