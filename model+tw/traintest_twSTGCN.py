...
def getModel():
    ks, kt, bs, T, n, p = 3, 3, [[channel, 16, 64], [64, 16, 64]], opt.his_len, num_variable, 0
    ...
    summary(model, (channel, opt.his_len, num_variable), device=device)
    return model
...
def testModel(name, mode, XS, YS, YS_multi):
    ...
    YS_multi, YS_pred_multi = np.squeeze(YS_multi[:, 0, :, :]), np.squeeze(YS_pred_multi[:, 0, :, :])
    ...
    f.write("%s, %s, Torch MSE, %.10e, %.10f\n" % (name, mode, torch_score, torch_score))
    logger.info("all pred steps, %s, %s, MSE, RMSE, MAE, MAPE, %.10f, %.10f, %.10f, %.10f\n" % (name, mode, MSE, RMSE, MAE, MAPE))
    f.write("all pred steps, %s, %s, MSE, RMSE, MAE, MAPE, %.10f, %.10f, %.10f, %.10f\n" % (name, mode, MSE, RMSE, MAE, MAPE))
    for i in range(opt.seq_len):
        MSE, RMSE, MAE, MAPE = Metrics.evaluate(YS_multi[:, i, :], YS_pred_multi[:, i, :])
        logger.info("%d step, %s, %s, MSE, RMSE, MAE, MAPE, %.10f, %.10f, %.10f, %.10f\n" % (i+1, name, mode, MSE, RMSE, MAE, MAPE))
...
parser.add_argument('--channel', type=int, default=2, help='how many channels')
...
def main():
    ...
    trainYS = mergeInfo(trainYS, trainYS_tw)
    ...
    testYS = mergeInfo(testYS, testYS_tw)
    ...
    testYS_multi = mergeInfo(testYS_multi, testYS_tw_multi)
    ...
    testModel(model_name, 'test', testXS, testYS, testYS_multi)
