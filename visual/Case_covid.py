...
def plot_con_mem(att_with:np.array, att_without:np.array):
    ...
    dates = ['12-'+str(x).zfill(2) for x in range(24, 32)] + ['01-'+str(x).zfill(2) for x in range(1, 7)]
    labels = []
...
if __name__ == '__main__':
    ...
    att_with = np.load(f'../model_ours/con_mem/covid-inflow_MemeSTN_202204261240/MemeSTN_att_score.npy')
    att_without = np.load(f'../model_ours/con_mem/covid-outflow_MemeSTN_202204261252/MemeSTN_att_score.npy')

    # test start: 10/06 10AM
    # +14h start: 10/07 0AM
    att_with = att_with[14:14+24*14, 4:5]
    att_without = att_without[14:14+24*14, ]
