...
class MemoryAGCRN(nn.Module):
    def __init__(self, num_nodes, input_dim, output_dim, horizon, rnn_units=64, num_layers=2, default_graph=True, embed_dim=8, cheb_k=2,
        ...
        self.encoder = AVWDCRNN(num_nodes, input_dim+tcov_h_dim*2, rnn_units, cheb_k, embed_dim, num_layers)     # mob
        # tcov input
    ...
    def forward(self, source, TE):
        ...
        X = torch.cat([X, TE[:,:-self.horizon,:,:], TE[:,-self.horizon:,:,:]], dim=-1)

        # mobility branch
        init_state = self.encoder.init_hidden(X.shape[0])
        output, _ = self.encoder(X, init_state, self.node_embeddings)      #B, T, N, hidden
        output = output[:, -1:, :, :]                                   #B, 1, N, hidden
        # output = torch.cat([output, TE[:,-self.horizon:,:,:].permute(0,3,2,1)], dim=-1)      # B, 1, N, hidden+horizon: enhance covid regular output
