...
class Graph_Convol(nn.Module):      # Graph Signal Propagation Rule
    def __init__(self, n_node:int, h_in:int, h_out:int, cheby_k:int, kappa:float, agg_opt:str, use_bias:bool=True):
        ...
        self.kappa = kappa      # todo: make trainable? GLU like?
    ...
    def forward(self, x:Tensor, G:Tensor):
        ...
        '''
        # todo: G discrete or softmax normalized
        # if discrete: random walk normalize
        # G preprocessed
        ...
        # todo: try purely random walk multiplication
...
class GCRU_Cell(nn.Module):       # Graph Convlutional Recurrent Unit
    def __init__(self, n_node:int, h_in:int, h_out:int, cheby_k:int, kappa:float=0.05, agg_opt:str='mixhop', use_bias:bool=True):    # todo: tune
...
class GCRN_Encoder(nn.Module):  # GCRN Encoder
    def __init__(self, n_node: int, h_in: int, h_out: int or list, cheby_k: int, n_layer: int, use_bias: bool = True,
                 st_norm: bool = False, pyramid: bool = False, factor: int = 2):
        ...
        self.st_norm = st_norm  # whether do ST-norm
        if self.st_norm:
            self.norm_list = nn.ModuleList()
        for i in range(self.n_layer):
            cur_in_dim = h_in if i == 0 else self.h_dim[i - 1] * (self.factor ** i)
            # cur_in_dim = h_in if i==0 else self.h_dim[i-1]*self.factor #(self.factor**i)
            self.cell_list.append(GCRU_Cell(n_node, cur_in_dim, self.h_dim[i], cheby_k, use_bias))
            if self.st_norm:
                self.norm_list.append(
                    nn.Sequential(nn.InstanceNorm2d(num_features=self.h_dim[i]), nn.LeakyReLU()))  # todo: adjust norm strategy
        ...
        for l in range(self.n_layer):
            ...
            if not self.pyramid:
                if not self.st_norm:
                    ...
                    in_seq_l = self.norm_list[l](out_seq_l.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            else:
                factor_list = []
                ...
                if not self.st_norm:
                    ...
                    out_seq_l = torch.cat(factor_list, dim=-1)
                    in_seq_l = self.norm_list[l](out_seq_l.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
                seq_len //= self.factor
    ...
    def _extend_for_multilayers(self, h: int):
        if not self.pyramid:
            ...
            h_list = [h] * self.n_layer
            # h_list = [h*(self.factor**i) for i in range(self.n_layer)]    # todo: try h_dim without factor
        return h_list
...
class GCRN_Decoder(nn.Module):  # GCRN Decoder
    ...
    def _extend_for_multilayers(self, h: int):
        if not self.pyramid:
            ...
            h_list = [h] * self.n_layer
            # h_list = [h*(self.factor**i) for i in range(self.n_layer)]    # todo: try h_dim without factor, which does not change decoder (may be the right way)
        return h_list


class GCRN(nn.Module):    # GCRN Encoder-Decoder Framework w/ ST-Memory
    def __init__(self, n_node:int, c_in:int, h_dim:int, cheby_k:int, n_layer:int, horizon:int, device:str, use_bias:bool=True, adp_graph:bool=True,
                 ...
                 st_norm:bool=False, pyramid:bool=True, factor:int=2):
        super(GCRN, self).__init__()
        self.horizon = horizon  # output steps
        self.device = device
        factor = factor if pyramid else 1

        # time_cov or st_memo: choose one
        self.time_cov = time_cov
        if not self.time_cov:
            self.encoder = GCRN_Encoder(n_node, c_in, h_dim, cheby_k, n_layer, use_bias, pyramid=pyramid, st_norm=st_norm)
            self.decoder = GCRN_Decoder(n_node, c_in, h_dim, cheby_k, n_layer, use_bias, pyramid=pyramid, st_norm=st_norm)
        else:
            self.encoder = GCRN_Encoder(n_node, c_in+tcov_h_dim, h_dim, cheby_k, n_layer, use_bias, pyramid=pyramid, st_norm=st_norm)
            self.decoder = GCRN_Decoder(n_node, c_in+tcov_h_dim, h_dim, cheby_k, n_layer, use_bias, pyramid=pyramid, st_norm=st_norm)
        ...
        if not self.st_memo:
            ...
            #self.flat_hidden = n_node * (h_dim*(factor**(n_layer-1)))
            self.flat_hidden = n_node * h_dim
            self.st_memory = self.construct_st_memory()
            #self.proj = nn.Sequential(nn.Linear(in_features=(h_dim*(factor**(n_layer-1)))+st_memo_dim, out_features=c_in, bias=use_bias), nn.ReLU())
    ...
    def init_adp_graph(self, n_node:int, emb_dim:int):
        '''
        node_idx = torch.arange(n_node).to(device)
        self.emb1 = nn.Embedding(n_node, emb_dim)
        self.emb2 = nn.Embedding(n_node, emb_dim)
        self.lin1 = nn.Linear(emb_dim, emb_dim)
        self.lin2 = nn.Linear(emb_dim, emb_dim)

        node_vec1 = self.emb1(node_idx)
        node_vec2 = self.emb2(node_idx)
        node_vec1 = self.lin1(node_vec1)
        node_vec2 = self.lin2(node_vec2)
        '''

        # todo: try embed one parameter matrix, multiply its transpose
    ...
    def construct_st_memory(self):      # MetaST-like: translated from TensorFlow
        # Global ST-Memory
        memory_weight = {}
        ...
        memory_weight['Wa'] = nn.Parameter(torch.randn(self.flat_hidden, self.st_memo_dim), requires_grad=True).to(self.device)     # todo: replace with Linear (+bias)
    ...
    def forward(self, x_seq:Tensor, t_x:Tensor=None, t_y:Tensor=None, G:Tensor=None):
        '''
        ...
        '''
        x_seq, tw_seq = torch.unsqueeze(x_seq[..., 0], -1), torch.unsqueeze(x_seq[..., -1], -1)  # channel 0: mob 1: twit
