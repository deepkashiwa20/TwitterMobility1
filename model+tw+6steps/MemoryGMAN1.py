...
class STEmbedding(nn.Module):
    ...
    def forward(self, SE, TE):
        # simple embedding
        SE = F.one_hot(torch.arange(SE.shape[0])).to(torch.float)
        ...
        TE[:, ...] = F.one_hot(torch.arange(TE.shape[1])).to(torch.float)
        # simple embedding
...
class MemoryGMAN(nn.Module):
    ...
    def __init__(self, SE, N, SE_dim, TE_dim, timestep_in, timestep_out, device, statt_layers=1, att_heads=8, att_dims=8, bn_decay=0.1,
        ...
        self.STAttBlock_2 = nn.ModuleList([STAttBlock(K, d, 0, bn_decay) for _ in range(L)])    # decoder
        
        self.transformAttention1 = transformAttention(K, d, 0, bn_decay)
        self.transformAttention2 = transformAttention(K, d, mem_dim, bn_decay)
        self.FC_1 = FC(input_dims=[1, D], units=[D, D], activations=[F.relu, None], bn_decay=bn_decay)
